...
declare module '@oarkflow/lithe/core' {
	export type Priority = 'sync' | 'userBlocking' | 'normal' | 'transition' | 'background' | 'idle';
	export interface SignalOptions { name?: string; equals?: false | ((previous: unknown, next: unknown) => boolean) }
	export interface ObserverOptions { sync?: boolean; priority?: Priority; name?: string }
	...
	export interface SuspenseController { register(promise: Promise<unknown>): void }
	export const SuspenseContext: { provide<R>(value: SuspenseController | null, fn: () => R): R; use(): SuspenseController | null; Provider(props: { value: SuspenseController | null; children?: unknown }): unknown };
	...
	export interface StoreApi<T> { getState(): T; setState(action: SetStateAction<T>, actionName?: string): void; patch(partialOrUpdater: Partial<T> | ((state: T) => Partial<T> | void), actionName?: string): void; setPath(path: readonly PropertyKey[], value: unknown, actionName?: string): void; mutate(producer: (draft: T) => void, actionName?: string): void; subscribe(listener: StoreListener<T>): () => void; select<U>(selector: (state: T) => U, equalityFn?: (a: U, b: U) => boolean): ReadonlySignal<U>; state: T; reset(): void; destroy(): void; onDestroy?(fn: () => void): () => void }
	...
	export function deviceProfile(): { memory: number | null; cores: number | null; saveData: boolean; connection: string; lowPower: boolean }; export function adaptivePriority(kind?: string): Priority | string; export function adaptiveSchedule<T>(task: () => T, kind?: string): Promise<T>; export function batteryProfile(): Promise<{ supported: boolean; low: boolean; charging?: boolean; level?: number }>; export function initBatteryAdaptation(): Promise<{ supported: boolean; low: boolean; charging?: boolean; level?: number }>; export function disposeBatteryAdaptation(): void; export function prefetchBudget(): { enabled: boolean; concurrency: number; distance: number }; export function createAdaptiveScheduler(options?: { concurrency?: number; maxPending?: number }): { schedule<T>(task: () => T, kind?: string): Promise<T>; dispose(): void; readonly profile: ReturnType<typeof deviceProfile>; readonly pending: number };
...
declare module '@oarkflow/lithe/signals' {
	export type { Priority, SignalOptions, ObserverOptions, Signal, ReadonlySignal } from '@oarkflow/lithe/core';
	export {
		Dependency,
		Observer,
		SignalImpl,
		...
		transition,
		scheduler
	} from '@oarkflow/lithe/core';
}

declare module '@oarkflow/lithe/dom' {
	import type { Signal, ReadonlySignal } from '@oarkflow/lithe/core';
	...
	export function mount(root: Element | ShadowRoot, view: VNode | (() => VNode), options?: { clear?: boolean; delegateEvents?: boolean }): () => void; export function createRoot(root: Element | ShadowRoot, options?: { delegateEvents?: boolean }): { render(view: VNode | (() => VNode)): void; unmount(): void }; export function hydrate(root: Element | ShadowRoot, view: VNode | (() => VNode), options?: { strict?: boolean; onMismatch?: (error: Error, report: HydrationReport) => void; delegateEvents?: boolean }): () => void;
	...
	export function __templateRecipe(html: string, withMarkers: boolean): { template: HTMLTemplateElement; markerPaths: Map<number, number[]> | null; attributePaths: Map<number, number[]> | null }; export function __resolveMarkerPath(fragment: unknown, path: number[]): unknown;
}

declare module '@oarkflow/lithe/router' {
...
declare module '@oarkflow/lithe/data' {
...
export class QueryClient { constructor(options?: { stale?: number | string; gc?: number | string; retry?: number; refetchOnFocus?: boolean; refetchOnReconnect?: boolean; persistence?: { save?(snapshot: unknown[]): void | Promise<void>; load?(): unknown[] | null | Promise<unknown[] | null> } | null; autoStart?: boolean }); fetchQuery<T = unknown>(options: QueryOptions<T>): Promise<T>; getQueryData<T = unknown>(key: QueryKey): T | undefined; setQueryData<T = unknown>(key: QueryKey, updater: T | ((old: T | undefined) => T), options?: { tags?: string[] }): T | undefined; invalidate(key: QueryKey): void; invalidateTags(tags: string | string[]): void; refetchStale(reason?: string): Promise<void>; dehydrate(): Array<{ key: string; data: unknown; updatedAt: number; tags: string[] }>; hydrate(snapshot?: Array<{ key: string; data: unknown; updatedAt?: number; tags?: string[] }>): void; persist(): Promise<void>; restore(): Promise<void>; clear(): void; destroy(): void; cancel(key: QueryKey): void; start(): () => void; subscribe(fn: (event: { type: string; key?: string; data?: unknown; error?: unknown; traceId?: string | null }) => void): () => void }
	...
	export function infiniteQuery<T = unknown, P = unknown>(options: { key: QueryKey; fetch?: (context: QueryContext) => T | Promise<T>; queryFn?: (context: QueryContext) => T | Promise<T>; getNextPageParam?: (lastPage: T, allPages: T[], lastPageParam: P) => P | undefined | null; initialPageParam: P; enabled?: boolean; select?: (pages: T[]) => unknown }): { readonly pages: T[]; readonly data: unknown; readonly loading: boolean; readonly error: unknown; readonly hasNext: boolean; fetchNext(): Promise<T | undefined>; refresh(): Promise<T>; key(): QueryKey; dispose(): void }; export function cursorQuery<T = unknown, P = unknown>(options: { key: QueryKey; fetch?: (context: QueryContext) => T | Promise<T>; queryFn?: (context: QueryContext) => T | Promise<T>; getNextPageParam?: (lastPage: T, allPages: T[], lastPageParam: P) => P | undefined | null; initialPageParam: P; enabled?: boolean; select?: (pages: T[]) => unknown }): { readonly pages: T[]; readonly data: unknown; readonly loading: boolean; readonly error: unknown; readonly hasNext: boolean; fetchNext(): Promise<T | undefined>; refresh(): Promise<T>; key(): QueryKey }; export function createStoragePersister(storage: { getItem(key: string): string | null | Promise<string | null>; setItem(key: string, value: string): void | Promise<void> }, key?: string): { load(): Promise<unknown[] | null>; save(snapshot: unknown[]): Promise<void> }; export function stream<T = unknown>(url: string, options?: { protocol?: 'websocket' | 'sse'; parse?: (data: string) => T; reconnect?: boolean; retryDelay?: number; maxRetryDelay?: number; autoConnect?: boolean; withCredentials?: boolean; onOpen?: () => void; onMessage?: (data: T) => void; onError?: (error: unknown) => void }): { data: import('@oarkflow/lithe/core').Signal<T | undefined>; error: import('@oarkflow/lithe/core').Signal<unknown>; connected: import('@oarkflow/lithe/core').Signal<boolean>; state: import('@oarkflow/lithe/core').Signal<string>; connect(): void; close(): void; reconnect(): void; send(value: unknown): void; dispose(): void }; export function request<T = unknown>(url: string, options?: RequestInit): Promise<T>; export function setDefaultFetcher(fetcher: (input: RequestInfo | URL, init?: RequestInit) => Promise<unknown>): void; export function parseDuration(value: unknown): number; export function stableQueryKey(key: unknown): string;
}

declare module '@oarkflow/lithe/forms' {
...
declare module '@oarkflow/lithe/rpc' {
	...
	export function handleServerFunction(request: Request, context?: any): Promise<Response>; export function serverManifest(): any; export function serverReference<TInput = any, TOutput = any>(moduleId: string, exportName?: string, options?: any): (input: TInput, options?: any) => Promise<TOutput>; export function createServerModuleHandler(manifest: any, options?: any): Promise<(request: Request, context?: any) => Promise<Response | null>>; export function getRegisteredServerFunction(id: string): any;
}

declare module '@oarkflow/lithe/server' {
	export * from '@oarkflow/lithe/rpc';
...
declare module '@oarkflow/lithe/offline' {
	export function createNetworkState(): any; export function registerServiceWorker(url?: string, options?: any): Promise<any>; export function createMutationQueue(storageKey?: string, options?: { maxItems?: number }): any; export function generateServiceWorker(options?: any): string;
	export function createMemoryStorage(): any; export function createIndexedDBStorage(options?: any): any; export function registerBackgroundSync(registration?: ServiceWorkerRegistration, tag?: string): Promise<boolean>; export function createPersistentMutationQueue(storage: any, key?: string, options?: { maxItems?: number }): any;
}

declare module '@oarkflow/lithe/sync' {
...
declare module '@oarkflow/lithe/collection' { export function collection<T extends Record<string, any>>(initial?: T[], options?: any): any; }
declare module '@oarkflow/lithe/virtual' { export function createVirtualizer(options: any): any; export function createGridVirtualizer(options: any): any; export function VirtualList(props: any): any; }
declare module '@oarkflow/lithe/grid' { export function createDataGrid(options: any): any; export function DataGrid(props: any): any; }
declare module '@oarkflow/lithe/style' { export function css(rules: Record<string, any>, options?: any): string; export function defineTheme(tokens: Record<string, any>, options?: any): any; export function collectedCSS(): string; export function clearCollectedCSS(): void; }
declare module '@oarkflow/lithe/image' { export function Image(props: any): any; export function transformImageURL(src: string, options?: any): string; export function transcodeImage(input: Blob | ImageBitmap | HTMLImageElement, options?: any): Promise<Blob>; export function supportsImageEncoding(type: string): Promise<boolean> | boolean; export function imageDimensions(input: any): Promise<{ width: number; height: number }>; }
declare module '@oarkflow/lithe/animation' { export function animate(element: Element, keyframes: Keyframe[] | PropertyIndexedKeyframes, options?: KeyframeAnimationOptions): Animation | any; export function transitionView(update: () => any, options?: any): any; export function spring(options?: any): any; }
declare module '@oarkflow/lithe/ui' { export function Dialog(props: any): any; export function Tabs(props: any): any; export function Disclosure(props: any): any; export function Menu(props: any): any; export function Listbox(props: any): any; export function Combobox(props: any): any; export function Tooltip(props: any): any; export function createToasts(): any; export function ToastRegion(props: any): any; export function Tree(props: any): any; export function CommandPalette(props: any): any; export function VisuallyHidden(props: any): any; }
declare module '@oarkflow/lithe/worker' { export interface WorkerFunction<TArgs extends any[] = any[], TReturn = any> { (...args: TArgs): Promise<Awaited<TReturn>>; withTransfer?(args: TArgs, transfer?: Transferable[]): Promise<Awaited<TReturn>>; terminate(): void } export function worker<TArgs extends any[], TReturn>(fn: (...args: TArgs) => TReturn | Promise<TReturn>, options?: any): WorkerFunction<TArgs, TReturn>; export function sharedWorker<TArgs extends any[], TReturn>(fn: (...args: TArgs) => TReturn | Promise<TReturn>, options?: any): WorkerFunction<TArgs, TReturn>; }
declare module '@oarkflow/lithe/interop' { export function defineElement(name: string, component: any, options?: any): CustomElementConstructor | null; export function foreign(mount: (host: Element, props: any) => void | (() => void)): any; export function createHostRenderer(adapter: any): any; export function createNativeRenderer(driver: any): any; export function createMemoryNativeDriver(): any; export function createReactBridge(React: any, ReactDOM: any): any; export function createVueBridge(Vue: any): any; export function createSvelteBridge(Svelte?: any): any; export function createExternalBridge(adapter: any): any; }
declare module '@oarkflow/lithe/devtools' { export function createDevtools(options?: any): any; export function mountReactiveGraphInspector(root: Element, options?: any): { render(): void; dispose(): void }; export function mountDevtoolsOverlay(root?: Element, options?: any): { render(): void; dispose(): void }; }
declare module '@oarkflow/lithe/observability' { export function trace(...args: any[]): any; export function traced(...args: any[]): any; export function onTrace(listener: Function): () => void; export function createTraceContext(...args: any[]): any; export function currentTraceContext(): any; export function withTraceContext<T>(ctx: any, fn: () => T): T; export function traceHeaders(...args: any[]): Record<string, string>; export function contextFromHeaders(headers: Headers | Record<string, string>): any; export function newCorrelationId(): string; export function currentCorrelation(): string | null; export function withCorrelation<T>(id: string | null, fn: (id: string) => T): T; export function correlationHeaders(id?: string | null): Record<string, string>; export function correlationFromHeaders(headers: Headers | Record<string, string>): string | null; export function onCorrelation(listener: Function): () => void; export function correlationEvent(type: string, attributes?: any, id?: string | null): any; }
declare module '@oarkflow/lithe/i18n' { export function createI18n(options: any): any; }
declare module '@oarkflow/lithe/head' { export function createHeadManager(options?: any): any; export function headToString(entries: any): string; }
declare module '@oarkflow/lithe/permissions' { export function createPermissions(source: any): any; export function Can(props: any): any; }
declare module '@oarkflow/lithe/testing' { export function render(view: any, options?: any): any; export function tick(fn?: Function): Promise<void>; export function click(element: Element): void; export function input(element: HTMLInputElement, value: any): void; export function assert(condition: any, message?: string): asserts condition; }
declare module '@oarkflow/lithe/app' { export interface AppConvention { root?: string | Element | DocumentFragment | null; component?: any; router?: any; routes?: any[]; server?: Record<string, unknown>; head?: unknown; errorBoundary?: any; adapters?: Record<string, unknown>; notFound?: any; mount?: Record<string, unknown>; devtools?: boolean } export interface NetworkState { readonly online: { readonly value: boolean; peek(): boolean }; readonly effectiveType: { readonly value: string; peek(): string }; readonly saveData: { readonly value: boolean; peek(): boolean }; start(): () => void; stop(): void } export interface AppInstance { readonly router: any; readonly network: NetworkState; dispose(): void } export function defineApp<T extends AppConvention>(config: T): Readonly<T>; export function startApp(config: AppConvention): AppInstance; }
declare module '@oarkflow/lithe/runtime' { export * from '@oarkflow/lithe/signals'; export function mount(root: Element | ShadowRoot, view: unknown, options?: { clear?: boolean; delegateEvents?: boolean }): () => void; export function createRoot(root: Element | ShadowRoot, options?: { delegateEvents?: boolean }): { render(view: unknown): void; unmount(): void }; }
declare module '@oarkflow/lithe' { export * from '@oarkflow/lithe/runtime'; }

declare module '@oarkflow/lithe/compiler' {
...
declare module '@oarkflow/lithe/experimental' {
	export { detectIslands, transformWorkerPlacement, reactiveGraphIR, optimizeReactiveGraph } from '@oarkflow/lithe/compiler';
	export { createHostRenderer, createNativeRenderer, createMemoryNativeDriver } from '@oarkflow/lithe/interop';
	export { createGridVirtualizer } from '@oarkflow/lithe/virtual';
}
declare module '@oarkflow/lithe/plugins' {
...
declare module '@oarkflow/lithe/vite' {
	import { litheVitePlugin } from '@oarkflow/lithe/plugins';
	export default litheVitePlugin;
	export { litheVitePlugin };
}
declare module '@oarkflow/lithe/rollup' {
	import { litheRollupPlugin } from '@oarkflow/lithe/plugins';
	export default litheRollupPlugin;
	export { litheRollupPlugin };
}
declare module '@oarkflow/lithe/babel' {
	import { litheBabelPlugin } from '@oarkflow/lithe/plugins';
	export default litheBabelPlugin;
	export { litheBabelPlugin };
}
declare module '@oarkflow/lithe/tailwind' {
	import { litheTailwindPlugin, compileTailwind } from '@oarkflow/lithe/plugins';
	export default litheTailwindPlugin;
	export { litheTailwindPlugin, compileTailwind };
}
declare module '@oarkflow/lithe/jsx-runtime' {
	export { jsx, jsxs, jsxDEV, Fragment } from '@oarkflow/lithe/dom';
...
declare module '@oarkflow/lithe/jsx-dev-runtime' {
	export * from '@oarkflow/lithe/jsx-runtime';
}
declare module '@oarkflow/lithe/dom/jsx-runtime' {
	export * from '@oarkflow/lithe/jsx-runtime';
}
declare module '@oarkflow/lithe/dom/jsx-dev-runtime' {
	export * from '@oarkflow/lithe/jsx-runtime';
}
