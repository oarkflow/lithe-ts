...
export async function prerenderProject(projectDir: string, options: PrerenderOptions = {}) {
	...
	if (!Array.isArray(routes)) {
		throw new Error(`${path.basename(config)} must export routes as an array or async function.`);
	}
	if (typeof mod.render !== 'function') {
		throw new Error(`${path.basename(config)} must export render(route) function.`);
	}
