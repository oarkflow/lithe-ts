...
async function replace(url,rec,callbacks){const registry=globalThis.__LITHE_HMR_SIGNAL_REGISTRY__;if(registry){const snap={};for(const [name,sig] of registry)try{snap[name]=sig.peek?.()??sig.value}catch{}globalThis.__LITHE_HMR_SIGNAL_SNAPSHOT__=snap;}for(const fn of rec?.dispose||[])try{await fn(hotData.get(rec.url))}catch(e){console.error('[lithe:HMR] dispose',e)}const fresh=await import(url+(url.includes('?')?'&':'?')+'t='+Date.now());for(const cb of callbacks)try{await cb(fresh)}catch(e){console.error('[lithe:HMR] accept',e)}return fresh}
