...
export async function analyzeProject(projectDir){
  ...
  let manifest=null;try{manifest=JSON.parse(await fs.readFile(path.join(target,'lithe-manifest.json'),'utf8'));}catch{}
  rows.sort((a,b)=>b.gzipBytes-a.gzipBytes); return {target,files:rows,total,gzip,manifest};
}
