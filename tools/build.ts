...
export async function buildProject(projectDir, options = {}) {
	...
	const sourceMaps = options.sourceMaps ?? config.sourceMaps ?? false, minify = options.minify ?? config.minify ?? true, dce = options.dce ?? config.dce ?? true;
