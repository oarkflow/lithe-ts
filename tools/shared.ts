...
const directExports = {
	'@lithe/core': { signal: 'core/reactive.js', computed: 'core/reactive.js', effect: 'core/reactive.js', batch: 'core/reactive.js', untrack: 'core/reactive.js', state: 'core/reactive.js', watch: 'core/reactive.js', isSignal: 'core/reactive.js', unwrap: 'core/reactive.js', onMutation: 'core/reactive-debug.js', inspectReactiveGraph: 'core/reactive-debug.js', serializeSignals: 'core/reactive-resume.js', restoreSignals: 'core/reactive-resume.js', installSignalSnapshot: 'core/reactive-resume.js', pendingSignals: 'core/reactive-resume.js', getNamedSignal: 'core/reactive-resume.js', schedule: 'core/scheduler.js', scheduler: 'core/scheduler.js', transition: 'core/scheduler.js', adaptiveSchedule: 'core/adaptive.js', deviceProfile: 'core/adaptive.js', adaptivePriority: 'core/adaptive.js', batteryProfile: 'core/adaptive.js', initBatteryAdaptation: 'core/adaptive.js', prefetchBudget: 'core/adaptive.js', createAdaptiveScheduler: 'core/adaptive.js', createScope: 'core/owner.js', onCleanup: 'core/owner.js', onMount: 'core/owner.js', createContext: 'core/owner.js', useContext: 'core/owner.js', getOwner: 'core/owner.js', withOwner: 'core/owner.js', disposeOwner: 'core/owner.js', createStore: 'core/store.js', defineStore: 'core/store.js', createContextStore: 'core/store.js', persist: 'core/store.js', history: 'core/store.js', devtools: 'core/store.js', produce: 'core/store.js' },
	'@lithe/dom': { h: 'dom/vnode.js', Fragment: 'dom/vnode.js', isVNode: 'dom/vnode.js', mount: 'dom/dom.js', dynamic: 'dom/dom.js', trustedHTML: 'dom/dom.js', staticTemplate: 'dom/dom.js', compiledTemplate: 'dom/dom.js', compiledElement: 'dom/dom.js', createElement: 'dom/dom.js', configureTrustedTypes: 'dom/dom.js', Show: 'dom/control.js', For: 'dom/control.js', Index: 'dom/control.js', Switch: 'dom/control.js', Match: 'dom/control.js', Dynamic: 'dom/control.js', Portal: 'dom/control.js', Island: 'dom/control.js', Await: 'dom/control.js', lazy: 'dom/control.js', lazyEvent: 'dom/control.js', hydrate: 'dom/hydrate.js', eventSymbol: 'dom/event-symbol.js', capturedEventSymbol: 'dom/event-symbol.js', resumeDocument: 'dom/resume.js', serializeResumeState: 'dom/resume.js' },
	...
	'@lithe/router': { createRouter: 'router/router.js', Link: 'router/router.js', group: 'router/router.js', prefetchPolicy: 'router/router.js', Outlet: 'router/utils.js', defineRoutes: 'router/utils.js', routePath: 'router/utils.js', routeManifest: 'router/utils.js', sharedTransition: 'router/utils.js' },
...
export function rewriteBareImports(code, prefix = '/__lithe/') {
	code = code.replace(/import\s*\{([^}]+)\}\s*from\s*(['"])((?:@lithe|lithe)\/[\w-]+)\2\s*;?/g, (full, body, q, spec) => {
		const canonical = spec.startsWith('lithe/') ? `@${spec}` : spec, map = directExports[canonical]; if (!map) return full;
		...
		if (fallback.length) { const target = aliases[canonical]; lines.push(`import { ${fallback.join(', ')} } from ${JSON.stringify(prefix + target)};`); }
		return lines.join('\n');
	});
	return code.replace(/((?:import|export)\s+(?:[^'";]+?\s+from\s+)?|import\s*\()(['"])((?:@lithe|lithe)\/[\w-]+)\2/g, (m, lead, q, spec) => {
		const target = aliases[spec] || aliases[spec.startsWith('lithe/') ? `@${spec}` : spec]; return target ? `${lead}${q}${prefix}${target}${q}` : m;
	});
}
