...
try{
  ...
  else if(command==='build'){ const result=await buildProject(arg,{outDir:flags.out}); console.log(`Built ${result.manifest.files.length} files (${result.manifest.bytes} bytes) -> ${result.out}`); }
