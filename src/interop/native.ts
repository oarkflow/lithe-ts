...
export function createNativeRenderer(driver) {
    return createHostRenderer(driver);
}
export function createMemoryNativeDriver() {
    ...
    return {
        ...
        insert(parent, node, before = null) {
            ...
            if (i >= 0) parent.children.splice(i, 0, node); else parent.children.push(node);
        },
        remove(node, parent = node?.parent) {
        ...
        snapshot(root) {
            const clean = n => n.type === '#text' ? n.value : {
                type: n.type,
                props: {
                    ...n.props
                },
