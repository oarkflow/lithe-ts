...
export type SetStateAction<T> =
	| Partial<T>
	| ((prev: T) => Partial<T> | void)
	| ((draft: T) => void);

export type StateCreator<T, Actions = {}> = (
	set: (action: SetStateAction<T>, actionName?: string) => void,
	get: () => T,
	store: StoreApi<T>
) => T & Actions;
 ...
 */
export function createStore<T, Actions extends object = {}>(
	creatorOrInitial: StateCreator<T, Actions> | (T & Actions) | T
): StoreHook<T & Actions> {
	let initialData: any = {};
	...
	if (typeof creatorOrInitial === 'function') {
		initialData = (creatorOrInitial as StateCreator<T, Actions>)(
			rawSetState,
			getState,
			dummyStore
		);
		...
		isScalar = true;
		initialData = { value: creatorOrInitial };
 ...
 */
export function defineStore<Id extends string, T extends object, Actions extends object = {}>(
	id: Id,
	creatorOrInitial: StateCreator<T, Actions> | (T & Actions)
): StoreHook<T & Actions> & { readonly $id: Id } {
	const store = createStore(creatorOrInitial);
	Object.defineProperty(store, '$id', { value: id, writable: false, enumerable: true });
	return store as StoreHook<T & Actions> & { readonly $id: Id };
}
 ...
 */
export function createContextStore<T extends object, P = Partial<T>>(
	factory: (props: P) => StoreHook<T>,
	options: { name?: string } = {}
): [
	Provider: (props: { value?: StoreHook<T>; initialProps?: P; children?: any }) => any,
	useStore: <U = T>(selector?: (state: T) => U) => U,
	Context: ReturnType<typeof createContext<StoreHook<T> | null>>
] {
	const Context = createContext<StoreHook<T> | null>(null, { name: options.name || 'StoreContext' });

	const Provider = (props: { value?: StoreHook<T>; initialProps?: P; children?: any }) => {
 ...
 */
export function persist<T extends object, Actions extends object = {}>(
	creator: StateCreator<T, Actions>,
 ...
 */
export function history<T extends object, Actions extends object = {}>(
	creator: StateCreator<T, Actions>,
	options: HistoryOptions = {}
): StateCreator<T, Actions & { undo(): void; redo(): void; canUndo(): boolean; canRedo(): boolean; clearHistory(): void }> {
 ...
 */
export function devtools<T extends object, Actions extends object = {}>(
	creator: StateCreator<T, Actions>,
	options: { name?: string } = {}
): StateCreator<T, Actions> {
	return (set, get, store) => {
		...
		if (extension) {
			devtoolsConnection = extension.connect({ name });
