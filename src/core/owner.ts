...
function hook(name: string, ...args: unknown[]): unknown {
    try {
        return globalThis.__LITHE_OWNER_HOOK__?.[name]?.(...args);
    } catch {
        return undefined;
    }
}
function createOwner(parent: Owner | null = currentOwner, options: OwnerOptions = {}): Owner {
    const owner: Owner = {
        id: ++ownerSeq,
        parent,
        cleanups: [],
...
export function createScope<T>(fn: (dispose: () => void) => T, options: OwnerOptions = {}): {
    value: T;
    dispose: () => void;
    owner: Owner;
} {
    ...
    try {
        return {
            value: withOwner(owner, () => fn(dispose)),
            dispose,
            owner
        };
    } catch (error) {
...
export function onMount(fn: () => void | (() => void)): void {
    if (!currentOwner) throw new Error('onMount() requires an active reactive scope.');
    const owner = currentOwner;
    queueMicrotask(() => {
        if (owner.disposed) return;
        const cleanup = withOwner(owner, fn);
        if (typeof cleanup === 'function' && !owner.disposed) owner.cleanups.push(cleanup);
    });
}
export function onCleanup<T extends () => void>(fn: T): T {
    if (!currentOwner) throw new Error('onCleanup() requires an active reactive scope.');
...
export function createContext<T>(defaultValue: T, options: {
    name?: string;
} = {}) {
...
export function __setOwnerSequence(value: number): void {
    ownerSeq = Math.max(ownerSeq, Number(value) || 0);
}
