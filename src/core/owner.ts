...
function createOwner(parent: Owner | null = currentOwner, options: OwnerOptions = {}): Owner { const owner: Owner = { id: ++ownerSeq, parent, cleanups: [], children: new Set<Owner>(), contexts: new Map<symbol, unknown>(), disposed: false, name: options.name || null, resume: options.resume || null }; owners.set(owner.id, owner); hook('create', owner); return owner; }
