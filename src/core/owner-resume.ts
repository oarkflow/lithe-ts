import { getOwner, __setOwnerSequence } from './owner.ts';
function safe(value, seen = new WeakSet()) {
    if (value == null || ['string', 'number', 'boolean'].includes(typeof value)) return value;
    if (typeof value === 'bigint') return String(value);
    if (value?.__litheSignal) return safe(value.peek?.() ?? value.value, seen);
    if (Array.isArray(value)) return value.map(v => safe(v, seen));
    if (typeof value === 'object') {
        if (seen.has(value)) return '[Circular]';
        seen.add(value);
        const out = {};
        for (const [k, v] of Object.entries(value)) if (typeof v !== 'function' && typeof v !== 'symbol') out[k] = safe(v, seen);
        seen.delete(value);
        return out;
    }
    return undefined;
...
export function serializeOwnerGraph(owner = getOwner()) {
    return ownerTree(owner);
}
export function createDetachedOwner(snapshot, parent = null) {
    ...
    for (const child of snapshot.children || []) owner.children.add(createDetachedOwner(child, owner));
    __setOwnerSequence(owner.id);
    return owner;
}
