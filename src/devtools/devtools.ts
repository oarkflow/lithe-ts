...
export function createDevtools(options={}){
  ...
  const installGlobal=(name='__LITHE_DEVTOOLS__')=>{if(typeof globalThis!=='undefined')globalThis[name]={history,mutations,inspectSignal,graph,toDOT,record,back:()=>step(-1),forward:()=>step(1),get cursor(){return cursor;}};return()=>{if(globalThis[name]?.history===history)delete globalThis[name];};};
  return{history,mutations,inspectSignal,graph,toDOT,record,back:()=>step(-1),forward:()=>step(1),installGlobal,dispose(){stopTrace();stopMutation();}};
}
