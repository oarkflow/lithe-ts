...
export async function renderRouterToString(router, request, options = {}) { const url = request instanceof Request ? new URL(request.url) : new URL(String(request), router.currentURL?.value || 'http://localhost/'); const target = router.resolve(url); const data = target.route ? await router.load(target, options.traceId) : undefined; const view = router.render(target, options.outlet || 'default'); return renderToString(view, { ...options, data, url, status: target.notFound ? 404 : 200 }); }

export function createSSRHandler({ router, render = renderRouterToString, headers = {}, ...options } = {}) { if (!router) throw new TypeError('createSSRHandler requires a router.'); return async request => { try { const target = router.resolve(new URL(request.url)); const html = await render(router, request, { ...options }); return new Response(html, { status: target.notFound ? 404 : 200, headers: { 'content-type': 'text/html; charset=utf-8', ...headers } }); } catch (error) { if (options.onError) options.onError(error, request); return new Response(options.errorHTML || 'Internal Server Error', { status: 500, headers: { 'content-type': 'text/html; charset=utf-8', ...headers } }); } }; }
