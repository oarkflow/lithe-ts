...
function splitTop(text: string, delimiter = ','): string[] {
    ...
    for (let i = 0; i < text.length; i++) {
        ...
        if (c === '(') round++; else if (c === ')') round--; else if (c === '[') square++; else if (c === ']') square--; else if (c === '{') curly++; else if (c === '}') curly--; else if (c === '<') angle++; else if (c === '>') angle = Math.max(0, angle - 1); else if (c === delimiter && round === 0 && square === 0 && curly === 0 && angle === 0) {
...
function union(types: any[]) {
    ...
    for (const t of types) {
        if (t.kind === 'union') flat.push(...t.types); else flat.push(t);
    }
...
export function parseType(text: string, env: Map<string, any> = new Map()): any {
    ...
    if (text.endsWith('[]')) return {
        kind: 'array',
        element: parseType(text.slice(0, -2), env)
    };
    let m = text.match(/^([A-Za-z_$][\w$]*)\s*<([\s\S]+)>$/);
    if (m) return {
        kind: 'generic',
        name: m[1],
        args: splitTop(m[2]).map(x => parseType(x, env))
    };
    if (text.startsWith('{') && text.endsWith('}')) {
        ...
        for (const part of splitTop(text.slice(1, -1), ';').flatMap(x => splitTop(x))) {
            ...
            if (p) {
                ...
                props[key] = {
                    optional: Boolean(p[2]),
                    type: parseType(p[3], env)
                };
            }
        }
        return {
...
function substitute(t: any, map: Map<string, any>): any {
    ...
    if (t.kind === 'array') return {
        ...t,
        element: substitute(t.element, map)
    };
    if (t.kind === 'union') return {
        ...t,
        types: t.types.map((x: any) => substitute(x, map))
    };
    if (t.kind === 'generic') return {
        ...t,
        args: t.args.map((x: any) => substitute(x, map))
    };
    if (t.kind === 'object') return {
        ...t,
        props: Object.fromEntries(Object.entries(t.props).map(([k, v]: [string, any]) => [k, {
            ...v,
            type: substitute(v.type, map)
        }]))
    };
    if (t.kind === 'function') return {
        ...t,
        params: t.params.map((x: any) => substitute(x, map)),
...
function resolve(t: any, env: Map<string, any>, seen = new Set<string>()): any {
    ...
    if (t.kind === 'generic') {
        ...
        if (def?.kind === 'genericDef' && !seen.has(t.name)) {
            seen.add(t.name);
            const map = new Map(def.params.map((p: string, i: number) => [p, t.args[i] || {
                kind: 'primitive',
                name: 'unknown'
            }]));
...
function inferObject(text: string, vars: Map<string, any>, env: Map<string, any>, functions: Map<string, any>) {
    ...
    for (const part of splitTop(inner)) {
        ...
        if (colon < 0) {
            const key = part.trim();
            if (/^[A-Za-z_$][\w$]*$/.test(key)) props[key] = {
                optional: false,
                type: vars.get(key) || {
                    kind: 'primitive',
                    name: 'unknown'
        ...
        const key = part.slice(0, colon).trim().replace(/^["']|["']$/g, ''),
            expr = part.slice(colon + 1);
        props[key] = {
            optional: false,
            type: inferExpression(expr, vars, env, functions)
        };
    }
    return {
        kind: 'object',
...
export function inferExpression(text: string, vars = new Map<string, any>(), env = new Map<string, any>(), functions = new Map<string, any>()): any {
    ...
    if (/^-?\d+(?:\.\d+)?(?:e[+-]?\d+)?$/i.test(expr)) return {
        kind: 'literal',
        value: Number(expr)
    };
    if (expr === 'true' || expr === 'false') return {
        kind: 'literal',
        value: expr === 'true'
    };
    if (expr === 'null') return {
    ...
    if (expr === 'undefined' || expr === 'void 0') return {
        kind: 'primitive',
        name: 'undefined'
    };
    if (expr.startsWith('[') && expr.endsWith(']')) {
        ...
        return {
            kind: 'array',
            element: parts.length ? union(parts.map(x => inferExpression(x, vars, env, functions))) : {
                kind: 'primitive',
                name: 'never'
            }
        };
    ...
    if (/^new\s+Map\b/.test(expr)) return {
        kind: 'generic',
        name: 'Map',
        args: [{
            kind: 'primitive',
    ...
    if (/^new\s+Set\b/.test(expr)) return {
        kind: 'generic',
        name: 'Set',
        args: [{
            kind: 'primitive',
    ...
    if (/^(?:async\s*)?(?:function\b|\([^)]*\)\s*=>|[A-Za-z_$][\w$]*\s*=>)/.test(expr)) return {
        kind: 'function',
        params: [],
        returns: {
            kind: 'primitive',
    ...
    if (/^[A-Za-z_$][\w$]*$/.test(expr)) return vars.get(expr) || env.get(expr) || {
        kind: 'primitive',
        name: 'unknown'
    };
    if (/[+]/.test(expr) && /['"`]/.test(expr)) return {
        kind: 'primitive',
        name: 'string'
    };
...
function collectTypes(source: string, env: Map<string, any>) {
    ...
    for (const m of source.matchAll(/\btype\s+([A-Za-z_$][\w$]*)(?:\s*<([^;=]+)>)?\s*=\s*/g)) {
        ...
        for (; end < source.length; end++) {
            const c = source[end];
            if (quote) {
                if (c === '\\') end++; else if (c === quote) quote = null;
                continue;
            }
            if (c === '"' || c === "'" || c === '`') {
                quote = c;
            ...
            if (c === '(') round++; else if (c === ')') round--; else if (c === '[') square++; else if (c === ']') square--; else if (c === '{') curly++; else if (c === '}') curly--; else if (c === '<') angle++; else if (c === '>') angle = Math.max(0, angle - 1); else if (c === ';' && round === 0 && square === 0 && curly === 0 && angle === 0) break;
        }
        const body = parseType(source.slice(i, end), env),
            params = m[2] ? splitTop(m[2]).map(x => x.trim().split(/\s+extends\s+/)[0].trim()) : [];
        env.set(m[1], params.length ? {
            kind: 'genericDef',
            params,
            body
...
function unsupportedTypeDiagnostics(source: string, file: string) {
    ...
    const patterns = [{
        ...
        code: 'TS_UNSUPPORTED_CONDITIONAL',
        message: 'Conditional types are syntax-valid, but Lithe semantic checking does not fully model them yet.'
    }, {
        re: /\binfer\s+[A-Za-z_$][\w$]*/g,
        code: 'TS_UNSUPPORTED_INFER',
        message: 'infer types are syntax-valid, but Lithe semantic checking does not fully model them yet.'
    }, {
        re: /\bkeyof\s+[A-Za-z_$][\w$]*/g,
        code: 'TS_UNSUPPORTED_KEYOF',
        message: 'keyof types are syntax-valid, but Lithe semantic checking treats them as named types.'
    }];
...
function widenLiteralType(t: any): any {
    ...
    if (t.kind === 'literal') {
        ...
        if (typeof t.value === 'bigint') return {
            kind: 'primitive',
            name: 'bigint'
        };
        if (typeof t.value === 'symbol') return {
            kind: 'primitive',
            name: 'symbol'
        };
    }
...
export function semanticTypecheck(source: string, options: {
    filename?: string;
    env?: Map<string, any>;
} = {}) {
    const file = options.filename || '<module>',
        env = options.env || collectTypeEnvironment([source]),
    ...
    for (const m of source.matchAll(/\b(?:export\s+)?(?:async\s+)?function\s+([A-Za-z_$][\w$]*)\s*(?:<[^>{}()]*>)?\s*\(([^)]*)\)\s*(?::\s*([^\{=>\n]+))?\s*\{/g)) {
        ...
        for (const p of splitTop(m[2])) {
            ...
            if (pm) params.push({
                name: pm[1],
                isRest,
                type: parseType(pm[2] || 'unknown', env)
            });
        }
        ...
        functions.set(m[1], {
            ...
            hasRest,
            restType,
            returns: parseType((m[3] || 'unknown').replace(/^Promise\s*<([\s\S]+)>$/, '$1'), env)
        });
    }
    for (const m of source.matchAll(/\b(?:const|let|var)\s+([A-Za-z_$][\w$]*)\s*:\s*([^=;\n]+)\s*=\s*([^;\n]+)/g)) {
        const expected = parseType(m[2], env),
            actual = inferExpression(m[3], vars, env, functions);
