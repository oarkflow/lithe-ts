...
function rewriteCapturedEvents(source, options = {}) {
	...
	while ((m = re.exec(source))) {
		...
		const refs = identifierRefs(info.body), params = new Set(info.params); for (const d of info.body.matchAll(/\b(?:const|let|var|function|class)\s+([A-Za-z_$][\w$]*)/g)) params.add(d[1]); for (const a of info.body.matchAll(/(?:\(([^)]*)\)|\b([A-Za-z_$][\w$]*))\s*=>/g)) for (const p of (a[1] || a[2] || '').split(',').map(x => x.trim()).filter(Boolean)) if (/^[A-Za-z_$][\w$]*$/.test(p)) params.add(p);
