...
function escapeClassName(cls: string): string {
    return cls.replace(/\\/g, '\\\\').replace(/\//g, '\\/').replace(/\[/g, '\\[').replace(/\]/g, '\\]').replace(/#/g, '\\#').replace(/%/g, '\\%').replace(/\./g, '\\.').replace(/:/g, '\\:').replace(/\(/g, '\\(').replace(/\)/g, '\\)').replace(/,/g, '\\,').replace(/&/g, '\\&').replace(/>/g, '\\>').replace(/\+/g, '\\+').replace(/~/g, '\\~').replace(/\*/g, '\\*');
}
    ...
    return {
        ...
        async transform(code: string, id: string) {
            if (id.endsWith('.css') && (code.includes('@tailwind') || code.includes('@import "tailwindcss"'))) {
                return {
                    code: await compileTailwind(code, options),
                    map: null
                };
            }
            return null;
