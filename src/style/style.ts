...
function kebab(key) { return key.replace(/[A-Z]/g,(m)=>`-${m.toLowerCase()}`); }
...
export function css(rules, options = {}) {
  ...
  for (const [key,value] of Object.entries(rules)) {
    ...
    if (key.startsWith('@media') || key.startsWith('@supports')) text += `${key}{${selector}{${declarations(value)}}}`;
    else if (key.startsWith('&')) text += `${key.replace('&',selector)}{${declarations(value)}}`;
    else text += `${selector} ${key}{${declarations(value)}}`;
  }
  sheets.set(name,text);
  if (typeof document !== 'undefined' && !document.querySelector(`style[data-lithe-style="${name}"]`)) {
    const el=document.createElement('style'); el.dataset.litheStyle=name; el.textContent=text; document.head.appendChild(el);
  }
  return name;
}
...
export function defineTheme(tokens, options = {}) {
  ...
  const lines=[];
  const walk=(obj,prefix=[])=>{ for(const [k,v] of Object.entries(obj)){ if(v&&typeof v==='object') walk(v,[...prefix,k]); else lines.push(`--${[...prefix,k].join('-')}:${v}`); } };
  walk(tokens);
...
export function collectedCSS() { return [...sheets.values()].join('\n'); }
export function clearCollectedCSS() { sheets.clear(); }
