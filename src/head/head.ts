...
function read(v) { return isSignal(v) ? v.value : typeof v === 'function' ? v() : v; }
...
export function createHeadManager() {
  ...
  const set = (descriptor) => {
    ...
    const disposers = [];
    if ('title' in descriptor) disposers.push(effect(() => { document.title = read(descriptor.title) || ''; }, {sync:true}));
    for (const meta of descriptor.meta || []) {
      const el = document.createElement('meta');
      for (const [k,v] of Object.entries(meta)) if (v != null) el.setAttribute(k,String(read(v)));
      el.dataset.litheHead = '1'; document.head.appendChild(el); owned.add(el);
      disposers.push(() => { owned.delete(el); el.remove(); });
    }
  ...
  const clear = () => { for (const el of owned) el.remove(); owned.clear(); };
  return { set, clear };
}
