...
export interface RouteDefinition {
    ...
    outlets?: Record<string, RouteComponent>;
    load?: (context: RouterContext) => unknown | Promise<unknown>;
    preload?: (context?: RouterContext) => unknown | Promise<unknown>;
...
export interface RouterOptions {
    ...
    notFound?: RouteDefinition | RouteComponent | null;
    fallback?: any;
    cacheEntries?: number;
...
export interface LinkProps {
    ...
    children?: any[];
    router?: any;
    prefetch?: boolean | 'auto' | 'visible' | 'idle';
...
export type TypedRouteDefinition<TParams extends Record<string, string> = Record<string, string>, TSearch = Record<string, unknown>, TData = unknown> = Omit<RouteDefinition, 'load' | 'preload' | 'component' | 'children'> & {
    ...
    load?: Loader<TData, RouterContext & {
        params: TParams;
        search: TSearch;
    }>;
    preload?: Loader<unknown, RouterContext & {
        params: TParams;
        search: TSearch;
    }>;
    children?: RouteDefinition[];
...
function newCorrelationId(): string {
    return globalThis.crypto?.randomUUID?.().replaceAll('-', '') || `${Date.now().toString(36)}${(++traceSeq).toString(36)}`;
}
function correlationEvent(type: string, attributes: Record<string, unknown> = {}, id: string | null = null): void {
    try {
        globalThis.__LITHE_CORRELATION_EVENT__?.(type, attributes, id);
...
function withCorrelation<T>(id: string | null | undefined, fn: (id: string) => T): T {
    const previous = globalThis.__LITHE_CORRELATION_ID__;
    globalThis.__LITHE_CORRELATION_ID__ = id || previous || newCorrelationId();
    let value;
    try {
        value = fn(globalThis.__LITHE_CORRELATION_ID__);
    } catch (error) {
        globalThis.__LITHE_CORRELATION_ID__ = previous;
        throw error;
    }
    if (value && typeof value.then === 'function') return value.finally(() => {
        if (globalThis.__LITHE_CORRELATION_ID__ === id) globalThis.__LITHE_CORRELATION_ID__ = previous;
    });
    globalThis.__LITHE_CORRELATION_ID__ = previous;
    return value;
}
...
function compilePattern(pattern: string): {
    ...
    const keys = [];
    const escaped = normalizePath(pattern).replace(/[.*+?^${}()|[\]\\]/g, '\\$&').replace(/:([A-Za-z_$][\w$]*)/g, (_, key) => {
        keys.push(key);
        return '([^/]+)';
...
function flattenRoutes(routes: RouteDefinition[], parentPath = '/', chain: RouteDefinition[] = [], out: any[] = []): any[] {
    for (const route of routes || []) {
        ...
        if (route.component || route.index || !route.children?.length) out.push({
            route,
            chain: next,
            fullPath,
            _compiled: compilePattern(fullPath),
            notFound: route.path?.includes('*')
        });
...
function matchFlat(entry: any, pathname: string): Record<string, string> | null {
    const match = normalizePath(pathname).match(entry._compiled.regex);
    if (!match) return null;
    const params = {};
    entry._compiled.keys.forEach((key, i) => params[key] = decodeURIComponent(match[i + 1] || ''));
    return params;
}
function searchObject(params: URLSearchParams): Record<string, string | string[]> {
    const out = {};
    for (const [k, v] of params) {
        if (k in out) out[k] = Array.isArray(out[k]) ? [...out[k], v] : [out[k], v]; else out[k] = v;
    }
    return out;
}
function memoryLimit(options: RouterOptions): number {
    ...
    if (memory && memory <= 2) return 2;
    if (memory && memory <= 4) return 4;
    if (memory && memory >= 8) return 12;
    return 6;
}
export function lazyRoute<TProps = any>(loader: () => Promise<any>, options: LazyRouteOptions = {}) {
    ...
    function LazyRoute(props: TProps) {
        ...
        return dynamic(() => {
            ...
            if (error) return options.fallback ?? null;
            return typeof options.fallback === 'function' ? options.fallback() : options.fallback ?? null;
        });
    }
    (LazyRoute as any).preload = load;
    return LazyRoute;
}
export function createRouter(optionsOrRoutes: RouterOptions | RouteDefinition[] = {}, {
    ...legacyOptions
    ...
    function resolve(urlLike) {
        ...
        for (const entry of flat) {
            const params = matchFlat(entry, url.pathname);
            if (params) candidates.push({
                entry,
                params
            });
        }
        ...
        if (!picked) {
            const fallback = typeof options.notFound === 'function' ? {
            ...
            return {
                ...
                search: searchObject(url.searchParams),
                url,
                notFound: true
            };
        }
        ...
        return {
            route,
            chain: picked.entry.chain,
            params: picked.params,
            query: url.searchParams,
            search,
            url,
            fullPath: picked.entry.fullPath,
            notFound: Boolean(picked.entry.notFound)
        };
    }
    ...
    async function loadResolved(target, traceId = null) {
        ...
        const run = async position => {
            ...
            const values = {};
            for (let i = 0; i < target.chain.length; i++) {
                const route = target.chain[i];
                if (!route.load) continue;
                values[i] = await withCorrelation(traceId, () => route.load({
                    ...context,
                    route,
                    target,
                    traceId
    ...
    async function prefetch(to: RouteTo): Promise<unknown> {
        ...
        try {
            ...
            prefetchCache.delete(next.href);
            throw error;
        }
    }
    async function navigate(to: RouteTo, navOptions: NavigateOptions = {}): Promise<unknown> {
        ...
        correlationEvent('navigation:start', {
        ...
        try {
            ...
            const commit = () => {
                if (typeof history !== 'undefined') {
                    ...
                    fn.call(history, navOptions.state || {}, '', next);
                }
                withCorrelation(traceId, () => batch(() => {
                    if (navOptions.modal || target.route?.intercept) backgroundURL.value = currentURL.value; else if (!navOptions.preserveBackground) backgroundURL.value = null;
                ...
                correlationEvent('navigation:end', {
                    to: next.href
                }, traceId);
            };
            ...
            if (typeof window !== 'undefined' && navOptions.scroll !== false) {
                const saved = navOptions.restore !== false && scrollPositions.get(next.href);
    ...
    function start(): () => void {
        ...
        const click = event => {
            ...
            const a = event.target.closest?.('a[data-lithe-link]');
    ...
    const api = {
        ...
        resolve,
        load: loadResolved,
        render: (target = resolve(currentURL.value), outlet = 'default') => compose(target, outlet),
        navigate,
        prefetch,
        start,
        View,
        ...
        invalidate(to) {
            if (to) {
                const href = new URL(to, currentURL.value).href;
                if (pageCache.has(href)) pageCache.get(href).stale = true;
            } else for (const e of pageCache.values()) e.stale = true;
        },
        get params() {
            return matched.value.params;
        },
        get query() {
            return matched.value.query;
        },
        get search() {
            return matched.value.search;
        },
        get path() {
            return currentURL.value.pathname;
        },
        get cache() {
            return pageCache;
        }
...
export function prefetchPolicy(): boolean {
...
export function defineLoader<TData = unknown, TContext extends RouterContext = RouterContext>(loader: Loader<TData, TContext>): Loader<TData, TContext> {
    return loader;
}
export function group(children: RouteDefinition[], options: Omit<RouteDefinition, 'children' | 'path'> & {
    path?: string;
} = {}): RouteDefinition {
    return {
        ...options,
...
export function Link(props: LinkProps): any {
    ...
    const ref = el => {
        ...
        observer = null;
        if (idle) {
            (globalThis.cancelIdleCallback || clearTimeout)(idle);
            idle = null;
        }
        if (!el || prefetch === false) return;
        if ((prefetch === 'auto' || prefetch === 'visible') && typeof IntersectionObserver !== 'undefined' && prefetchPolicy()) {
        ...
        if (prefetch === 'idle' && prefetchPolicy()) {
            const run = () => warm();
            idle = (globalThis.requestIdleCallback || (fn => setTimeout(fn, 120)))(run, {
                timeout: 1500
            });
        }
    };
