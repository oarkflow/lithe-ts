...
export interface RouteDefinition { path?:string; name?:string; component?:RouteComponent; layout?:RouteComponent; children?:RouteDefinition[]; outlets?:Record<string,RouteComponent>; load?:(context:any)=>unknown|Promise<unknown>; preload?:()=>unknown|Promise<unknown>; searchSchema?:{parse(value:unknown):any}; intercept?:boolean; cache?:boolean|string|number; index?:boolean }
export interface RouterOptions { routes?:RouteDefinition[]; initialURL?:string|URL; notFound?:RouteDefinition|RouteComponent|null; fallback?:any; cacheEntries?:number; viewTransitions?:boolean }
...
function flattenRoutes(routes:RouteDefinition[],parentPath='/',chain:RouteDefinition[]=[],out:any[]=[]):any[] {
  for(const route of routes||[]) {
    ...
    if(route.component||route.index||!route.children?.length) out.push({route,chain:next,fullPath,_compiled:compilePattern(fullPath)});
...
export function createRouter(optionsOrRoutes:RouterOptions|RouteDefinition[]={},{...legacyOptions}:RouterOptions={} as RouterOptions) {
  ...
  function resolve(urlLike) {
    ...
    candidates.sort((a,b)=>b.entry.fullPath.length-a.entry.fullPath.length);
    const picked=candidates[0];
    if(!picked) return {route:options.notFound||null,chain:[],params:{},query:url.searchParams,search:searchObject(url.searchParams),url};
    ...
    return {route,chain:picked.entry.chain,params:picked.params,query:url.searchParams,search,url,fullPath:picked.entry.fullPath};
  }
  ...
  async function loadResolved(target,traceId=null) {
    ...
    if(cached&&!cached.stale) {cached.used=Date.now();return cached.data;}
    const result={};
    for(let i=0;i<target.chain.length;i++){
      const route=target.chain[i]; if(!route.load)continue;
      result[i]=await withCorrelation(traceId,()=>route.load({params:target.params,query:target.query,search:target.search,url:target.url,route,target,traceId}));
    }
    const data=target.chain.length?result[target.chain.length-1]:undefined;
  ...
  async function prefetch(to:RouteTo):Promise<unknown> {
    ...
    const promise=(async()=>{for(const r of target.chain)await r.preload?.();return loadResolved(target,newCorrelationId());})();
  ...
  const api={routes,flat,currentURL,navigation,backgroundURL,matched,navigate,prefetch,start,View,Outlet,
