...
test('build splits .server modules out of browser output and generates callable stubs',async()=>{
  ...
  await fs.writeFile(path.join(root,'src/actions.server.ts'),`import { server } from '@lithe/rpc'; export const secret=server(async input=>({value:input.x+1,marker:'VERY_PRIVATE_MARKER'}));`);
