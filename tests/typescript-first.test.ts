...
test('project generator creates TypeScript/TSX source',async()=>{
  ...
  try{
    ...
    await fs.access(path.join(dir,'src/index.tsx'));
    await assert.rejects(fs.access(path.join(dir,'public')));
    ...
    const main=await fs.readFile(path.join(dir,'src/index.tsx'),'utf8');
    assert.match(main,/signal<number>/);
    assert.match(main,/@oarkflow\/lithe['"];/);
...
test('typed TSX builds to browser JavaScript without unresolved TypeScript imports',async()=>{
  ...
  try{
    ...
    await fs.writeFile(path.join(dir,'index.html'),'<!doctype html><div id="app"></div><script type="module" src="/src/index.tsx"></script>');
    ...
    await fs.writeFile(path.join(dir,'src/index.tsx'),`import { signal } from '@oarkflow/lithe/signals'; import { mount } from '@oarkflow/lithe/dom'; import { seed, type Item } from './model.ts'; const current=signal<Item>(seed); function App(){return <strong>{()=>current.value.value}</strong>} mount(document.getElementById('app')!,<App/>);`);
    ...
    const main=await fs.readFile(path.join(out,'src/index.js'),'utf8');
